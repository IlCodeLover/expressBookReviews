...
regd_users.put("/auth/review/:isbn", (req, res) => {
  //Write your code here
    ...
    if (books[isbn]) {
        ...
        return res.status(200).send("Review  is successfully added");
    }
    else {
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
    ...
    if (books[isbn]) {
        let book = books[isbn];
        delete book.reviews[username]; // delete specific user's review.
        return res.status(200).send("Review is deleted successfully.");
    }
    else {
        return res.status(404).json({message: `Book with ISBN ${isbn} is not found`});
    }
